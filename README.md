...
> cat Anopheles-gambiae-PEST_BASEFEATURES_AgamP4.10.gtf | sed '1,5d' | grep -v transcript_id | cut -f 1,4,5,9 | cut -f 1 -d ";" | sed 's/gene_id //' | sed 's/"//g' > Anopheles-gambiae-PEST_BASEFEATURES_AgamP4.8.genes.bed
...
> bedtools coverage -counts -a Anopheles-gambiae-PEST_BASEFEATURES_AgamP4.8.genes.bed -b SRR830370_smallsubset.GEM.Agambiae.sorted.samtools.filtered.diffs.vcf > genes.SRR830370_smallsubset.GEM.Agambiae.sorted.samtools.filtered.diffs.vcf.counts.txt
...
- SRR1805322_sub_1.fastq : reads from transcriptome of adult male of A.gambiae (first of paired-end pair)

- SRR1805322_sub_2.fastq : reads from transcriptome of adult male of A.gambiae (second of paired-end pair)

- SRR1805328_sub_1.fastq : reads from transcriptome of adult female of A.gambiae (first of paired-end pair)

- SRR1805328_sub_2.fastq : reads from transcriptome of adult female of A.gambiae (second of paired-end pair)

----------------------

Index reference transcriptome for GEM (takes about 10 minutes)
...
> gemtools rna-pipeline -i Anopheles-gambiae-PEST_CHROMOSOMES_AgamP4.gem -a Anopheles-gambiae-PEST_BASEFEATURES_AgamP4.8.gtf -f SRR1805322_sub_1.fastq SRR1805322_sub_2.fastq -q 33 -t 2 --no-bam

then convert the resulting file to SAM

> zcat SRR1805322_sub_1.map.gz | gem-2-sam -q offset-33 -o SRR1805322_sub_1.sam -I Anopheles-gambiae-PEST_CHROMOSOMES_AgamP4.gem -m -l -T 2
...
The files *.gtf.counts.txt contain the gene counts therein.
...
> sort -k 1 .gtf.counts.txt > .gtf.counts.sorted.txt
